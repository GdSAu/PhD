...
def chamfer_distance(dir_carpeta, itera):
    ...
    points1 = o3d.io.read_point_cloud( dir_carpeta + "Point_cloud/"+ itera + "cloud_gt.pcd").voxel_down_sample(voxel_size=0.0035)
    points1 = np.asarray(points1.points)
    points2 = np.asarray(o3d.io.read_point_cloud( dir_carpeta + "Point_cloud/"+ itera + "cloud_acc.pcd").points)
...
def  Get_surface_coverage_gain(direccion,i, itera, umbral=0.005): # 0.0037 #P_t - P_t-1 < umbral
    
    pt = o3d.io.read_point_cloud(direccion + "Point_cloud/"+ itera + "cloud_{}.pcd".format(i), remove_nan_points=True, remove_infinite_points = True)
    pt_1 = o3d.io.read_point_cloud(direccion + "Point_cloud/"+ itera + "cloud_{}.pcd".format(i-1), remove_nan_points=True, remove_infinite_points = True)
...
def getCobertura(dir_carpeta,carpeta_iter,i, umbral=0.001):
    '''
    P_acu = nube acumulada
    W_obj = nube del objeto
    '''
    if i> 0:
        points1 = o3d.io.read_point_cloud( dir_carpeta + "Point_cloud/"+ carpeta_iter + "cloud_gt.pcd")
        points1 = points1.voxel_down_sample(voxel_size=umbral)
        points2 = o3d.io.read_point_cloud( dir_carpeta + "Point_cloud/"+ carpeta_iter + "cloud_acc.pcd")
